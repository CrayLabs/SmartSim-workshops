...
## Setup
As a first step, you will need to `git clone` this repo. Given it is a small repo, you can do it on your home directory. Then you will need to navigate to the directory named `LUMI_2026` (the one containing this `README.md` file).
...
> **NOTE** This paragraph can be skipped if you are on LUMI and use the provided virtual environment and SmartRedis installation.
...
If you want to work on the C, C++, or Fortran version of the simulation, and if you are not working on LUMI, you will need to build the SmartRedis clients, following [these instructions](https://www.craylabs.org/docs/installation_instructions/basic.html#build-smartredis-library-c-c-fortran) (you can also clone the current `develop` branch instead of the stable release).
...
If you are on `LUMI`, please follow the next step, if you are on your laptop, just skip it.

### Getting the allocation on Slurm
To launch jobs on `LUMI`, you will need to work within an interactive *allocation* given by the workload manager, Slurm. For this tutorial, you will only need two nodes.
